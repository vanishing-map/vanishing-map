...
### 2026-06-27 연령·성별 유량 필터 추가
- **정한 것 / 한 것**: 행안부 OD API를 다시 호출해 총 이동량, 성별, 연령대, 연령대별 성별 이동량을 포함한 wide CSV를 만들고 `index.html`의 유량 계산 기준에 연결했다.
- **왜**: 단순 화면 필터가 아니라 선택한 연령·성별 집단의 실제 이동량으로 arc 굵기, 순위, 허브 크기, 지표가 다시 계산되어야 하기 때문이다.
- **어떻게**: `data/mois-ppltn-data-stus-sido-od-202605-age-gender.csv`와 interregional CSV를 추가했다. 화면에는 연령(`전체`, `0-14`, `15-24`, `25-39`, `40-54`, `55-64`, `65+`)과 성별(`전체`, `남성`, `여성`) 선택을 넣고, `people`, `male`, `female`, `age_*`, `age_*_male`, `age_*_female` 컬럼을 조합해 유량을 산출하도록 했다.
- **막힌 점 / 바꾼 점**: 기존 CSV에는 `male/female`만 있고 연령 컬럼이 없어 데이터 재수집이 필요했다.
- **배운 것 / 다음**: 이후 색상은 현재 유량 기준과 별개로 지역 특성, 정책 유형, 관계인구 특성 같은 속성 레이어에 배정할 수 있다.
